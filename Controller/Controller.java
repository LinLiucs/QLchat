...
public class Controller {
        ...
        ((Button) $(register,"code")).setOnAction(event -> {
            ...
            try{
                ...
                if (!phone.equals("") && Pattern.matches(phoneRegExp,phone)){
                    codeForget = new GetMessageCode().smsCode();
    ...
    public void registerExec(){
        ((Button) $(register,"register")).setOnAction(event -> {
            ...
            if(nickname.equals("") || password.equals("") || name.equals("") || rePassword.equals("") || birthday.equals("") || phone.equals("") || code.equals("")){
                ...
                String account = (int) ((Math.random() * 9 + 1) * 10000) + "";
                try {
                    ...
                    if(!resultSet.next()){
                        if(password.equals(rePassword)){
                            ...
                            database.exec("insert into user values(?,?,?,?,?,?,?,?,?,?,?)",account,nickname,name,password,birthday,sex,userdata.getHead(),"河南省信阳市","",phone,"background6");
                ...
                try {
                    if(resultSet.next()){
                        if(resultSet.getString("password").equals(password)){
                            ...
                            if(set.next()){
                                dialog.setErrorTip("accountError","该账号已经登录!!!");
                            }else {
                                ...
                                /*设置聊天助手*/
                                try {
                                    mainwindow.addFriend("助手小强","head9","助手小强","快来找我聊天吧!");
                                    ((Button) $(Controller.ch,"friendMore")).setText("助手小强");
                                }catch (IOException e){
                                    e.printStackTrace();
                                }
                                messageData.msg.add(new Vector<>());
                                messageData.account.add("助手小强");
                                messageData.msgTip.put("助手小强",0);
                                /*把所有好友添加到列表*/
    ...
    public void sendfile() {
        ((Button) $(ch,"file")).setOnAction (event -> {
            FileChooser fileChooser = new FileChooser();
            fileChooser.setTitle("选择上传文件");
            File s1=fileChooser.showOpenDialog(null);
            String s2 = s1.getAbsolutePath();
            ...
            try {
                ...
                DatagramPacket datagramPacket = new DatagramPacket(bytes,length, InetAddress.getByName("127.0.0.1"),10000);
    ...
    public void sendMsg(){
        ((Button) $(ch,"send")).setOnAction(event -> {
            ...
            try{
                ...
                if(resultSet.next() && !remark.equals("助手小强")){
            ...
            if(!text.equals("")){
                try{
                    if (Y_account.equals("助手小强")){
                        try {
                            String xiong = "你好，我是小刘,本系统使用的帮助为:.............................";
